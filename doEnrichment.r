...
pngname=paste(arg[2],'.png',sep="")
pdfname=paste(arg[2],'.pdf',sep="")
...
#############plot(as.vector(seq(from=1,to=nrow(mymat)))/nrow(mymat), computmat[,1], type="l",lwd=4.5,yaxt="n",xaxt="n",col="green",xlab=xtitle,ylab=ytitle,cex.lab=2,cex.axis=2,ann=FALSE)
#lines(computmat[,1])
...
############################dev.off(); 
#saveToPDF(pdfname)
#saveToPNG(pngname,width=1500,height=1500,res=300)
