...
${homeDir}/getInteractionsInHashWithNewDictionaryAndGroup_ENS.pl $pathways $NameToENSDict $out
...
sed -i.ba -e 's|\"|U+0022|g' -e 's|'\''|U+2019|g' ${out}.ensids.grouped
rm ${out}.ensids.grouped.ba
sed -i.ba -e 's|\"|U+0022|g' -e 's|'\''|U+2019|g' ${out}.genenames.grouped
rm ${out}.genenames.grouped.ba
...
#awk -F"\t" '{if (f==1) { if(NF<=1001&&NF>=11){r[$1]} } else if (( $1 in r)) {print}}' f=1 ${out}.ensids.grouped f=2 ${out}.ensids.grouped > ${out}.ensids.grouped.10-1000

#awk -F"\t" '{if (f==1) {if(NF<=1001&&NF>=11){r[$1]} } else if (( $1 in r)) {print}}' f=1 ${out}.ensids.grouped f=2 ${out}.genenames.grouped > ${out}.genenames.grouped.10-1000
